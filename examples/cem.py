...
writer = tf.summary.FileWriter('./data/cem_hard')
...
def do_rollout(agent, env, num_steps, render=False):
    ...
    for t in range(num_steps):
        ...
        with open("./file_data/cem.csv", "a") as f:
            f.write("{}, {}\n".format(t, reward))
        writer.add_summary(sess.run(merged, feed_dict={r: reward}))
        total_rew += reward
...
if __name__ == '__main__':
    ...
    # env.spec = gym.envs.registration.EnvSpec("PegInsertion-v0")
    env.seed(0)
    np.random.seed(0)
    params = dict(n_iter=10, batch_size=25, elite_frac = 0.2)
    num_steps = 1000

    outdir = './gym_data/cem-agent-results'
    wrappers.Monitor(directory=outdir, force=True)(env)
